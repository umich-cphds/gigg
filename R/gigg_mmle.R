...
#' @param a A (G x 1) column vector of shape parameter for the prior on the group shrinkage parameters.
#' @param b A (G x 1) column vector of inital shape parameter for the prior on the individual shrinkage parameters.
...
#'                          stable_solve = FALSE)
grouped_igg_mmle <- function(X, C, Y, grp_idx, alpha_inits = rep(0, ncol(C)), beta_inits = rep(0, ncol(X)), lambda_sq_inits = rep(1, ncol(X)), gamma_sq_inits = rep(1.0, length(unique(grp_idx))), a = rep(0.5, length(unique(grp_idx))), b = rep(0.5, length(unique(grp_idx))),
